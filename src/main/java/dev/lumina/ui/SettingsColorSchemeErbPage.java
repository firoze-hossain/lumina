...
/**
 * Editor > Color Scheme > ERB settings page.
 * Replicates 1:1 visual and functional parity with reference screenshot media_1790428949903.png:
 * - Scheme header row with dynamic switcher, actions menu, theme link, and help icon.
 * - Flat Category Tree view with 7 attributes:
 *   (Comment, Expression end, Expression start, Omit new line modifier,
 *    Ruby injection background, Scriptlet end, Scriptlet start).
 * - Default selection: "Comment" (foreground #BC9458, italic checked, inherits Comment from (HTML) unchecked).
 ...
 * - Live Interactive ERB Code Editor Preview with syntax tokens, token selection box,
 *   right margin guideline, and bidirectional click synchronization.
 */
public class SettingsColorSchemeErbPage extends VBox {
    ...
    private String currentInheritedTargetKey = "Comment";
    private String currentInheritedScope = "(HTML)";
    private String foregroundHex = "#BC9458";
    ...
    public SettingsColorSchemeErbPage() {
        ...
        inheritTargetLabel = new Label("Comment");
        ...
        inheritScopeLabel = new Label("(HTML)");
    ...
    private void buildCategoryTree() {
        ...
        // 7 flat attributes matching media_1790428949903.png
        root.getChildren().addAll(
                new TreeItem<>("Comment"),
                new TreeItem<>("Expression end"),
                new TreeItem<>("Expression start"),
                new TreeItem<>("Omit new line modifier"),
                new TreeItem<>("Ruby injection background"),
                new TreeItem<>("Scriptlet end"),
                new TreeItem<>("Scriptlet start")
        );
    ...
    private void initListeners() {
        ...
        effectsCheck.setOnAction(e -> {
            if (effectsCheck.isSelected() && effectsHex == null) {
                effectsHex = foregroundHex != null ? foregroundHex : "#BC9458";
            }
            onAttributeControlChanged();
        });
    ...
    private void loadAttributesForSelectedKey() {
        ...
        try {
            ...
            if (hasInherit) {
                ...
                currentInheritedScope = desc.getInheritScope() != null ? desc.getInheritScope() : "(HTML)";
    ...
    private void updatePreview() {
        ...
        // ERB snippet matching media_1790428949903.png:
        // <%# Sample comment %>
        // <% 3.times do { -%>
        //   <%= "Hello" + " world!" %> <br />
        // <% } %>
        ...
        List<List<Token>> lines = List.of(
                List.of(new Token("<%# Sample comment %>", "Comment", "#BC9458")),
                List.of(
                        new Token("<%", "Scriptlet start", "#CF8E6D"),
                        new Token(" 3.times do { ", null, "#BCBEC4"),
                        new Token("-", "Omit new line modifier", "#CC7832"),
                        new Token("%>", "Scriptlet end", "#CF8E6D")
                ),
                List.of(
                        new Token("  ", null, "#BCBEC4"),
                        new Token("<%=", "Expression start", "#CF8E6D"),
                        new Token(" \"Hello\" + \" world!\" ", null, "#BCBEC4"),
                        new Token("%>", "Expression end", "#CF8E6D"),
                        new Token(" <br />", null, "#E8BF6A")
                ),
                List.of(
                        new Token("<%", "Scriptlet start", "#CF8E6D"),
                        new Token(" } ", null, "#BCBEC4"),
