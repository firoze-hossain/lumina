...
public class SettingsLessPage extends VBox {
    public SettingsLessPage() {
        ...
        String[] items = {"At-keyword", "Attribute name", "Bad character", "Braces", "Brackets",
            "Class name", "Colon", "Comma", "Comment", "Dot", "Function",
            "Hex color", "Id selector", "Identifier", "Important"};
        ...
        for (String s : settings) {
            if (s.equals("Foreground") || s.equals("Background") || s.equals("Error stripe mark")) {
                ...
                color.setStyle("-fx-text-fill: #1E1F22; -fx-font-size: 14px;");
        ...
        inheritCombo.getItems().addAll("Colored (CSS)", "Language Defaults");
        inheritCombo.getSelectionModel().selectFirst();
        inheritCombo.getStyleClass().add("settings-combo");
        inheritCombo.setPrefWidth(200);
        CheckBox colored = new CheckBox("Colored");
        colored.setSelected(true);
        colored.getStyleClass().add("settings-check");
        inheritRow.getChildren().addAll(inheritLabel, inheritCombo, colored);
        ...
        Label p1 = new Label("@import 'mixins'; // external mixins");
        p1.setStyle("-fx-text-fill: #6AAB73;");
        Label p2 = new Label("@the-border: 1px;");
        p2.setStyle("-fx-text-fill: #CF8E6D;");
        Label p3 = new Label("@base-color: #111;");
        p3.setStyle("-fx-text-fill: #CF8E6D;");
        Label p4 = new Label("#header {");
        p4.setStyle("-fx-text-fill: #56A8F5;");
        Label p5 = new Label("    color: @base-color * 3;");
