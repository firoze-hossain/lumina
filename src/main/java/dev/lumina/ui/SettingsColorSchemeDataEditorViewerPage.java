...
/**
 * Editor > Color Scheme > Data Editor and Viewer settings page.
 * Replicates 1:1 visual and functional parity with reference screenshot media_1790427959950.png:
 * - Scheme header row with dynamic switcher, actions menu, theme link, and help icon.
 * - Grouped Category Tree view:
 *   Data Grid:
 *     - Alternating row color
 *     - Error data (default selected, Error stripe mark #D64D5B, Effects #FA6675 Underwaved, inherits Errors and Warnings->Error from (General))
 *     - Image data
 *     - Null data
 * - Full Attribute Editor panel with Error stripe mark, Effects dropdown (Underwaved), and "Inherit values from".
 * - Live Interactive Data Grid Preview with simulated table rows (null, image pill badge, error cell with underwave and row highlight),
 *   right margin error stripe bar, and bidirectional click synchronization.
 */
public class SettingsColorSchemeDataEditorViewerPage extends VBox {
    ...
    // Preview
    private final VBox previewTableBox;
    private final VBox errorStripeGutter;
    private final ScrollPane previewScrollPane;

    // State
    private String selectedKey = "Error data";
    private String currentInheritedTargetKey = "Errors and Warnings->Error";
    private String currentInheritedScope = "(General)";
    private String foregroundHex = null;
    private String backgroundHex = null;
    private String errorStripeHex = "#D64D5B";
    private String effectsHex = "#FA6675";
    ...
    public SettingsColorSchemeDataEditorViewerPage() {
        ...
        inheritTargetLabel = new Label("Errors and Warnings->Error");
        ...
        // 4. Live Data Grid Preview Pane
        previewTableBox = new VBox();
        previewTableBox.setStyle("-fx-background-color: #1E1F22;");
        HBox.setHgrow(previewTableBox, Priority.ALWAYS);

        previewScrollPane = new ScrollPane(previewTableBox);
        ...
        errorStripeGutter.setPadding(new Insets(60, 2, 0, 2));
    ...
    private void buildCategoryTree() {
        ...
        // Group "Data Grid"
        TreeItem<String> gridGroup = new TreeItem<>("Data Grid");
        gridGroup.setExpanded(true);
        gridGroup.getChildren().addAll(
                new TreeItem<>("Alternating row color"),
                new TreeItem<>("Error data"),
                new TreeItem<>("Image data"),
                new TreeItem<>("Null data")
        );
    ...
    private String getCompositeKey(String shortKey) {
        return "Data Grid // " + shortKey;
    }
    ...
    private void loadAttributesForSelectedKey() {
        ...
        try {
            ...
            String compKey = getCompositeKey(selectedKey);
            ...
            if (hasInherit) {
                ...
                currentInheritedScope = desc.getInheritScope() != null ? desc.getInheritScope() : "(General)";
            ...
            effectTypeCombo.setValue(attr != null && attr.getEffectType() != EffectType.NONE ? attr.getEffectType() : EffectType.UNDERWAVED);
    ...
    private void updatePreview() {
        previewTableBox.getChildren().clear();
        errorStripeGutter.getChildren().clear();

        EditorColorSchemeSettings settings = EditorColorSchemeSettings.getInstance();
        String scheme = headerBar.getSelectedScheme();

        ColorAttribute altAttr = "Alternating row color".equals(selectedKey) ? getCurrentEditorAttribute() : settings.getAttribute(scheme, "Data Grid // Alternating row color");
        ColorAttribute errAttr = "Error data".equals(selectedKey) ? getCurrentEditorAttribute() : settings.getAttribute(scheme, "Data Grid // Error data");
        ColorAttribute imgAttr = "Image data".equals(selectedKey) ? getCurrentEditorAttribute() : settings.getAttribute(scheme, "Data Grid // Image data");
        ColorAttribute nullAttr = "Null data".equals(selectedKey) ? getCurrentEditorAttribute() : settings.getAttribute(scheme, "Data Grid // Null data");

        String altBg = (altAttr != null && altAttr.getBackground() != null) ? altAttr.getBackground() : "#26282B";
        String errEffectColor = (errAttr != null && errAttr.getEffectColor() != null) ? errAttr.getEffectColor() : "#FA6675";
        String errStripeColor = (errAttr != null && errAttr.getErrorStripeColor() != null) ? errAttr.getErrorStripeColor() : "#D64D5B";

        // Table Header
        HBox headerRow = new HBox();
        headerRow.setAlignment(Pos.CENTER_LEFT);
        headerRow.setStyle("-fx-background-color: #212225; -fx-border-color: #393B40; -fx-border-width: 0 0 1 0;");
        headerRow.setPadding(new Insets(4, 8, 4, 8));

        Label thNum = new Label("#");
        thNum.setPrefWidth(40);
        thNum.setStyle("-fx-text-fill: #848BA3; -fx-font-size: 11px;");

        Label thVal = new Label("Value");
        thVal.setStyle("-fx-text-fill: #848BA3; -fx-font-size: 11px;");

        headerRow.getChildren().addAll(thNum, thVal);
        previewTableBox.getChildren().add(headerRow);

        // Row 1: null data
        HBox row1 = new HBox();
        row1.setAlignment(Pos.CENTER_LEFT);
        row1.setPadding(new Insets(6, 8, 6, 8));
        row1.setStyle("-fx-background-color: #1E1F22; -fx-cursor: hand;" + ("Null data".equals(selectedKey) ? " -fx-border-color: #3574F0; -fx-border-width: 1;" : ""));
        row1.setOnMouseClicked(e -> selectTreeItem("Null data"));

        Label r1Num = new Label("1");
        r1Num.setPrefWidth(40);
        r1Num.setStyle("-fx-text-fill: #6F737A; -fx-font-size: 12px;");

        Label r1Val = new Label("null");
        String nullColor = (nullAttr != null && nullAttr.getForeground() != null) ? nullAttr.getForeground() : "#7A7E85";
        r1Val.setStyle("-fx-text-fill: " + nullColor + "; -fx-font-style: italic; -fx-font-size: 12px; -fx-font-family: 'Monospaced';");
        row1.getChildren().addAll(r1Num, r1Val);
        previewTableBox.getChildren().add(row1);

        // Row 2: Alternating row + Image data
        HBox row2 = new HBox();
        row2.setAlignment(Pos.CENTER_LEFT);
        row2.setPadding(new Insets(6, 8, 6, 8));
        String row2Border = ("Alternating row color".equals(selectedKey) || "Image data".equals(selectedKey)) ? " -fx-border-color: #3574F0; -fx-border-width: 1;" : "";
        row2.setStyle("-fx-background-color: " + altBg + "; -fx-cursor: hand;" + row2Border);
        row2.setOnMouseClicked(e -> selectTreeItem("Image data"));

        Label r2Num = new Label("2");
        r2Num.setPrefWidth(40);
        r2Num.setStyle("-fx-text-fill: #6F737A; -fx-font-size: 12px;");

        Label r2Val = new Label("400×300 PNG image 7.14K");
        String imgFg = (imgAttr != null && imgAttr.getForeground() != null) ? imgAttr.getForeground() : "#BCBEC4";
        String imgBg = (imgAttr != null && imgAttr.getBackground() != null) ? imgAttr.getBackground() : "#393B40";
        r2Val.setStyle("-fx-text-fill: " + imgFg + "; -fx-background-color: " + imgBg + "; -fx-background-radius: 3; -fx-border-color: #4E5157; -fx-border-radius: 3; -fx-padding: 2 6 2 6; -fx-font-size: 11px; -fx-font-family: 'Monospaced';");
        row2.getChildren().addAll(r2Num, r2Val);
        previewTableBox.getChildren().add(row2);

        // Row 3: Error data (Selected row matching screenshot)
        HBox row3 = new HBox();
        row3.setAlignment(Pos.CENTER_LEFT);
        row3.setPadding(new Insets(6, 8, 6, 8));
        String row3SelectedBorder = "Error data".equals(selectedKey) ? " -fx-border-color: #3574F0; -fx-border-width: 1;" : "";
        row3.setStyle("-fx-background-color: #2E436E; -fx-cursor: hand;" + row3SelectedBorder);
        row3.setOnMouseClicked(e -> selectTreeItem("Error data"));

        Label r3Num = new Label("3");
        r3Num.setPrefWidth(40);
        r3Num.setStyle("-fx-text-fill: #DFE1E5; -fx-font-size: 12px;");

        Label r3Val = new Label("<failed to load>");
        r3Val.setStyle("-fx-text-fill: #DFE1E5; -fx-font-size: 12px; -fx-font-family: 'Monospaced'; -fx-border-color: transparent transparent " + errEffectColor + " transparent; -fx-border-style: segments(2, 2); -fx-border-width: 0 0 2 0;");
        row3.getChildren().addAll(r3Num, r3Val);
        previewTableBox.getChildren().add(row3);

        // Error stripe in right gutter
        Rectangle errStripe = new Rectangle(8, 4, Color.web(errStripeColor));
        errorStripeGutter.getChildren().add(errStripe);
    }
    ...
    private ColorAttribute getCurrentEditorAttribute() {
        ...
        if (effectsCheck.isSelected() && effectsHex != null) {
            ...
            attr.setEffectType(effectTypeCombo.getValue() != null ? effectTypeCombo.getValue() : EffectType.UNDERWAVED);
        }
        return attr;
    }
    ...
    public void apply() {
        ...
        settings.setAttribute(scheme, getCompositeKey(selectedKey), attr);
        modified = false;
        notifyModified();
    }
    ...
    public VBox getPreviewTableBox() { return previewTableBox; }
}
